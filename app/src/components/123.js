const example1Customer = {
  ...
  ListOfPeriods: "   15.03.2011237 - 01.05.2017",
  OpenCredits: ".00",
  OpenDebts: ".00",
...
const extractDates = (customer) => {
  if (
    ...
    const allPeriods = customer.ListOfPeriods.split("|");
    let extractedDates = "";
    const arrOfDates = [];
    ...
    allPeriods.map((per) => {
      ...
      extractedDates = per.replaceAll(" ", "").split("-");
      console.log("AA", extractedDates);
      if (extractedDates[0].length === 10 && extractedDates[1].length === 10) {
        Dates.startDate = extractedDates[0];
        Dates.endDate = extractedDates[1];
        arrOfDates.push(Dates);
      } else {
        arrOfDates.push("string length is invalid");
      }
    });
    const lengthErr = arrOfDates.find(
      (ar) => ar === "string length is invalid"
    );
    if (lengthErr) {
...
console.log("FUN", extractDates(example1Customer));
